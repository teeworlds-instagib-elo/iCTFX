...
void CGameContext::ConFlagPass(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int Lobby = pResult->m_Lobby;
	if(Lobby == 0)	//Lobby 0 is save server
	...
	pSelf->m_apController[pResult->m_Lobby]->m_flag_pass = !pSelf->m_apController[pResult->m_Lobby]->m_flag_pass;
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(!pSelf->PlayerExists(i) || pSelf->GetLobby(i) != Lobby)
			continue;
		...
		char aBuf[256];
		str_format(aBuf, 256, "Flag passing is %s", pSelf->m_apController[pResult->m_Lobby]->m_flag_pass ? "enabled" : "disabled");
...
void CGameContext::ConGrenadeHook(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int Lobby = pResult->m_Lobby;
	if(Lobby == 0)	//Lobby 0 is save server
	...
	pSelf->m_apController[pResult->m_Lobby]->m_grenade_hook = !pSelf->m_apController[pResult->m_Lobby]->m_grenade_hook;
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(!pSelf->PlayerExists(i) || pSelf->GetLobby(i) != Lobby)
			continue;
		...
		char aBuf[256];
		str_format(aBuf, 256, "grenade hook is %s", pSelf->m_apController[pResult->m_Lobby]->m_grenade_hook ? "enabled" : "disabled");

		pSelf->SendChatTarget(i, aBuf);
	}
}

void CGameContext::ConLaserCollision(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int Lobby = pResult->m_Lobby;
	if(Lobby == 0)	//Lobby 0 is save server
	...
	pSelf->m_apController[pResult->m_Lobby]->m_laser_collision = !pSelf->m_apController[pResult->m_Lobby]->m_laser_collision;
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(!pSelf->PlayerExists(i) || pSelf->GetLobby(i) != Lobby)
			continue;
		...
		char aBuf[256];
		str_format(aBuf, 256, "laser collision is %s", pSelf->m_apController[pResult->m_Lobby]->m_laser_collision ? "enabled" : "disabled");

		pSelf->SendChatTarget(i, aBuf);
	}
}

void CGameContext::ConGrenadeAmmo(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int Lobby = pResult->m_Lobby;
	if(Lobby == 0)	//Lobby 0 is save server
	...
	pSelf->m_apController[Lobby]->m_grenade_ammo = pResult->GetInteger(0);
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(!pSelf->PlayerExists(i) || pSelf->GetLobby(i) != Lobby)
			continue;
		...
		char aBuf[256];
		str_format(aBuf, 256, "grenade ammo is set to %i", pSelf->m_apController[Lobby]->m_grenade_ammo);
...
void CGameContext::ConXonX(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(Lobby == 0)	//Lobby 0 is save server
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			pSelf->SendChatTarget(i, "You cannot start a match in lobby 0, got a different lobby");
		}
		return;
	}
