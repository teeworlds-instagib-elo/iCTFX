...
SqlHandler::SqlHandler() :
	m_thread_pool(get_proc_count()) { m_should_stop.store(true); }
...
void SqlHandler::threadloop()
{
	...
	m_queue.appendListener(EventType::CreatePlayer, make_callback<GetPlayerStatsData>([](GetPlayerStatsData *data, IDbConnection *database) {
	...
	m_queue.appendListener(EventType::SetStats, make_callback<SetStatsData>([](SetStatsData *data, IDbConnection *database) {
		char aError[256] = "error message not initialized";
		if(database->Connect(aError, sizeof(aError)))
		{
			dbg_msg("sql", "failed connecting to db: %s", aError);
	...
	m_queue.appendListener(EventType::SetServerStats, make_callback<SetServerStatsData>([](SetServerStatsData *data, IDbConnection *database) {
		char aError[256] = "error message not initialized";
		if(database->Connect(aError, sizeof(aError)))
		{
			dbg_msg("sql", "failed connecting to db: %s", aError);
