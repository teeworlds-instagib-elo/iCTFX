...
IDbConnection CGameControllerDDRace::GetDatabaseConnection() {
	return CreateMysqlConnection("ddnet", "record", "ddnet", "thebestpassword", "localhost", 3306, true);
}
	...
	IGameController(pGameServer), m_Teams(pGameServer)
{
	...
	if(g_Config.m_SvSaveServer) {
		io.post([](){
			auto database = GetDatabaseConnection();
			if(database != nullptr)
			{
