...
int CGameControllerDDRace::OnCharacterDeath(class CCharacter *pVictim, class CPlayer *pKiller, int WeaponID)
{
	...
	int team = m_Teams.m_Core.Team(pKiller->GetCID());

	// drop flags
	for(int i = 0; i < 2; i++)
	{
		CFlag *F = m_apFlags[team][i];
...
void CGameControllerDDRace::Snap(int SnappingClient)
{
	...
	pGameDataObj->m_TeamscoreRed = m_aTeamscore[team][TEAM_RED];
	pGameDataObj->m_TeamscoreBlue = m_aTeamscore[team][TEAM_BLUE];

	if(m_apFlags[team][TEAM_RED])
	{
		if(m_apFlags[team][TEAM_RED]->m_AtStand)
			pGameDataObj->m_FlagCarrierRed = FLAG_ATSTAND;
		else if(m_apFlags[team][TEAM_RED]->m_pCarryingCharacter && m_apFlags[team][TEAM_RED]->m_pCarryingCharacter->GetPlayer())
			pGameDataObj->m_FlagCarrierRed = m_apFlags[team][TEAM_RED]->m_pCarryingCharacter->GetPlayer()->GetCID();
		else
	...
	{
		if(m_apFlags[team][TEAM_BLUE]->m_AtStand)
			pGameDataObj->m_FlagCarrierBlue = FLAG_ATSTAND;
		else if(m_apFlags[team][TEAM_BLUE]->m_pCarryingCharacter && m_apFlags[team][TEAM_BLUE]->m_pCarryingCharacter->GetPlayer())
			pGameDataObj->m_FlagCarrierBlue = m_apFlags[team][TEAM_BLUE]->m_pCarryingCharacter->GetPlayer()->GetCID();
		else
	...
	for(int team = 0; team < 64; team++)
	{
		for(int fi = 0; fi < 2; fi++)
		{
			...
			if(F->m_pCarryingCharacter)
			{
				...
				if(m_apFlags[fi^1] && m_apFlags[team][fi^1]->m_AtStand)
				{
					if(distance(F->m_Pos, m_apFlags[team][fi^1]->m_Pos) < CFlag::ms_PhysSize + CCharacter::ms_PhysSize)
					{
						// CAPTURE! \o/
						m_aTeamscore[team][fi^1] += 100;
		...
		if(m_GameOverTick[team] == -1 && !m_Warmup)
		{
			...
			if((g_Config.m_SvScorelimit > 0 && (m_aTeamscore[team][TEAM_RED] >= g_Config.m_SvScorelimit || m_aTeamscore[team][TEAM_BLUE] >= g_Config.m_SvScorelimit)) ||
				(g_Config.m_SvTimelimit > 0 && (Server()->Tick()-m_RoundStartTick[team]) >= g_Config.m_SvTimelimit*Server()->TickSpeed()*60))
			{
				if(m_SuddenDeath[team])
				{
					if(m_aTeamscore[team][TEAM_RED]/100 != m_aTeamscore[team][TEAM_BLUE]/100)
						EndRound(team);
				}
				else
				{
